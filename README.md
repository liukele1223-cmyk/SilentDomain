...
静域是一个面向无网络场景的离线私密聊天应用。它不需要服务器、账号、手机号或互联网：两台附近的手机通过蓝牙低功耗（BLE）发现并建立加密通信通道，即可直接收发文字消息。

> **当前版本仅适配 Android，不支持 iPhone / iPad 或其他 iOS 设备。** 应用最低支持 **Android 7.0（API 24）及以上**，当前以 **Android 16（API 36）** 为目标版本编译；已在 Android 14 与 Android 16 真机完成附近设备发现、验证码确认和 200～400 字文字消息收发验证。
...
- 图片与表情跨设备传输：通过加密 BLE 分片双向发送，提供进度、送达、失败重试、全屏缩放预览和透明图片支持。
- 本地表情管理：相册批量导入、固定分页、搜索、时间/名称/大小排序、正倒序、多选删除与聊天附件清理。
- 聊天图片历史：每条新图片消息均在本机加密消息库中保留快照；删除表情或清理附件后，聊天记录仍可显示原图。
- 本地表情导入会移除 EXIF、重新编码并加密保存在本机。
...
前置条件：Flutter SDK、Android SDK，以及一台运行 Android 7.0（API 24）或更高版本、已开启 USB 调试或无线调试的 Android 手机。
...
1. 两台 Android 手机都安装相同版本的静域，并打开蓝牙。
2. 在手机 A 首页点击“开始广播”。
...
表情使用：在聊天输入框左侧点击笑脸图标，选择“从相册批量导入”。可在表情面板或“设置 → 表情与附件”中进入管理页；接收图片不会自动加入“我的表情”，只有在预览中主动保存后才会加入。图片和透明表情可通过已建立的加密 BLE 通道双向发送。

聊天图片会保存为本机加密历史快照。因此后续删除本地表情或清理聊天附件，不会影响当前版本之后收发图片的聊天记录展示。
...
自动测试覆盖启动页、设备发现流程、文字与图片消息状态、表情管理与历史快照、本地存储、BLE 帧重组、UUID 协议以及 ECDH/AES-GCM 加密与篡改拒绝行为。
...
- `image_picker` / `image`：系统相册选择、图片重新编码与 EXIF 移除
